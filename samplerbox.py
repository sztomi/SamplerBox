...
MAX_POLYPHONY = 90  # This can be set higher, but 80 is a safe value
    ...
    while 1:
      ...
      if chunkname == b"fmt ":
        ...
        (
          ...
          numsampleloops,
          samplerdata,
        ) = struct.unpack("<iiiiiiiii", chunk.read(36))
...
class Sound:
  def __init__(self, filename, midinote, velocity):
    ...
    self.data = self.frames2array(
      wf.readframes(self.nframes), wf.getsampwidth(), wf.getnchannels()
    )
...
FADEOUTLENGTH = 30000
FADEOUT = numpy.linspace(1.0, 0.0, FADEOUTLENGTH)  # by default, float64
FADEOUT = numpy.power(FADEOUT, 6)
FADEOUT = numpy.append(FADEOUT, numpy.zeros(FADEOUTLENGTH, numpy.float32)).astype(
  numpy.float32
)
        ...
        try:
          ...
          for fname in os.listdir(dirname):
            ...
            if m:
              ...
              if notename:
                midinote = (
                  NOTES.index(notename[:-1].lower()) + (int(notename[-1]) + 2) * 12
                )
